...
const Card = ({ props }) => {
  ...
  const handleApply = async (id)=>{
    const res = await axios.post('/api/student/apply/'+id, { withCredentials: true , credentials: 'include'})
    console.log(res);
  }
  return (
    <div className=''>
      <div className="border-r-2 rounded-tl-lg rounded-tr-lg bg-white w-48 py-4 px-4 min-w-[500px]">
        ...
        <button onClick={()=>handleApply(_id)} className="w-full bg-green-500 font-semibold mb-12 rounded-br-lg rounded-bl-lg py-1">Apply</button>
    </div>
  )
}
